/*
 * taskpool.hpp
 * C++ streams API — FIFO priority task pool on the service Asio executor.
 */
#pragma once
...
class PriorityTaskPoolImpl final : public IPriorityTaskPool {
 private:
  ...
  struct SharedState final {
    SharedState(std::string poolName, IServiceEnvironment& environment)
      ...
      if (!pool) {
        throw std::invalid_argument("priority task pool configuration named '" + name +
                                    "' not found");
      }
      ...
      gaugeExecutorsBusy = scope->gauge(
          "executors_busy", "Number of priority task pool executors running callbacks");
      ...
      stopTimeoutCounter =
          scope->counter("events_total", "Total number of events in priority task pool",
                         {{"event", "stop_timeout"}});
      taskRejectedCounter =
          scope->counter("events_total", "Total number of events in priority task pool",
                         {{"event", "task_rejected"}});
      taskExpeditedCounter =
          scope->counter("events_total", "Total number of events in priority task pool",
    ...
    std::unique_ptr<metrics::Int64Counter> taskExpeditedCounter;
  };
 ...
 public:
  ...
  void start(Context ctx) override {
    ...
    if (!pool || pool->executorsCount <= 0) {
      throw std::runtime_error("failed to start priority task pool '" + state->name +
                               "'");
    }
  ...
  void addTask(Context ctx, int priority,
    ...
    task->sequence = state->nextSequence++;
 ...
 private:
  ...
  static void expedite(const std::weak_ptr<SharedState>& weakState,
    ...
    task->promoted = true;
    bestEffort([state] { state->taskExpeditedCounter->inc(); });
  }
...
inline std::unique_ptr<IPriorityTaskPool> makePriorityTaskPool(std::string name,
                                                               IServiceEnvironment& env) {
